...
Graylog2.prototype.log = function (level, msg, meta, callback) {
    ...
    zlib.deflate(new Buffer(JSON.stringify(message)), function(err, compressedMessage) {
        if (err) {
        ...
        self.udpClient.send(compressedMessage, 0, compressedMessage.length, self.graylogPort, self.graylogHost, function (err, bytes) {
           if (err) {
               callback(err, null);
           } else {
               callback(null, true);
